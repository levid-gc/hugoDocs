...
title: 内容组织
linktitle: 组织
description: Hugo 假设源码内容与渲染的站点具有相同的组织结构。
date: 2017-02-01
...
自 Hugo `0.32` 后，与页面相关的图片以及其他的资源都将被打包进 `Page Bundles`。

这些概念都是相互关联的，因此，您可能需要阅读 [页面资源]({{< relref "/content-management/page-resources" >}}) 和 [图像处理]({{< relref "/content-management/image-processing" >}}) 以获取更全面的理解。
...
在 Hugo 中，您的内容的组织方式应能反映出选渲染后的站点的组织方式。

虽然 Hugo 支持任意层级的内容嵌套，顶级目录（比如，`content/<DIRECTORIES>`）在 Hugo 中具有特殊用途，它被认为是内容的类型的分片并被用于确定布局，等等。 想要了解更多有关分区的内容，以及如何嵌套它们，参见 [分区][sections]。
